...
class MarketWorker:
    def __init__(
        ...
        *,
        ...
        self._fill_window: deque[tuple[float, float]] = deque(maxlen=20)
    ...
    def exposure_px(self) -> float | None:
        ...
        if not self._fill_window:
            return None
        notional = sum(px * s for px, s in self._fill_window)
    ...
    async def tick(self, fleet_paused: bool) -> None:
        ...
        if breaker:
            ...
            if abs(pos) < m.min_size_step:
                await self.cancel_both_sides()
                return
            # holding inventory: the reducing side keeps working below
